...
parms <- infections %>% 
  ...
  select(-m) %>%
  crossing(
      bind_rows(crossing(scenario = "pcr",
...
res2 %>% 
  ...
  ggplot(aes(x = fct_relevel(scenario, "pcr", "rapid", "quarantine_fast", "quarantine_slow")
  ...
  facet_grid2(~symptom_isolation, axes="all",
              labeller = labeller(symptom_isolation = function(x) paste("Isolation upon symptom onset:", tolower(x))))+
  scale_colour_brewer(palette = "Set2")+
  scale_shape_manual(values=c(15:19),na.value = 19)+
  lims(y = c(0, 1)) +
